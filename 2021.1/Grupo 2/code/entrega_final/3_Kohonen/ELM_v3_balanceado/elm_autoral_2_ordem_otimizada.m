...
function H = kernel_matrix_2_ordem(ActivationFunction, NumberofHiddenNeurons, NumberofTrainingData, InputWeight, P, BiasofHiddenNeurons)

%%%%%%%%%%% Calculate hidden neuron output matrix H
switch lower(ActivationFunction)	   
	case {'abertura'}   
		[ay,ax] = size(P); % Atributos de entrada
		[by,bx] = size(Htemp); %Temporário
		ay
		by
		ax
		bx
		
		Htemp = kernel_matrix('ero_classica', size(InputWeight,2), NumberofTrainingData, InputWeight, P, BiasofHiddenNeurons);
		H = kernel_matrix('dil_classica', NumberofHiddenNeurons, NumberofTrainingData, InputWeight, Htemp, BiasofHiddenNeurons);
	case {'fechamento'}   
		Htemp = kernel_matrix('dil_classica', size(InputWeight,2), NumberofTrainingData, InputWeight, P, BiasofHiddenNeurons);
		H = kernel_matrix('ero_classica', NumberofHiddenNeurons, NumberofTrainingData, InputWeight, Htemp, BiasofHiddenNeurons);		
end		
